...
**Purpose:** Comprehensive list of missing API endpoints and modules before implementation
...
| Module | Status | Completion | Priority | Database Schema |
|--------|--------|------------|----------|----------------|
| **Authentication** | ✅ Complete | 100% | ✅ Done | ✅ Complete |
| **Users** | ✅ Complete | 100% | ✅ Done | ✅ Complete |
| **Patients** | ✅ Complete | 100% | ✅ Done | ✅ Complete |
| **Visits** | ✅ Complete | 100% | ✅ Done | ✅ Complete |
| **Prescriptions** | ❌ Missing | 0% | 🔴 CRITICAL | ✅ Exists |
| **Billing/Invoices** | ❌ Missing | 0% | 🔴 CRITICAL | ✅ Exists |
| **Reports** | ❌ Missing | 0% | 🟡 HIGH | ✅ Uses existing data |
| **Audit Logs** | ❌ Missing | 0% | 🟡 HIGH | ✅ Exists |
| **Files** | ❌ Missing | 0% | 🟡 HIGH | ⚠️ Needs metadata storage |

### Overall Completion: **44%** (4 of 9 core modules)
...
**Priority:** 🔴 **CRITICAL**  
**Database Schema:** ✅ Exists in Prisma (`Prescription` model)
...
- ✅ `medicationName`, `genericName`, `brandName`
- ✅ `dosage`, `frequency`, `route`, `duration`, `quantity`
- ✅ `refills`, `instructions`
- ✅ `status` (ACTIVE, DISCONTINUED, COMPLETED)
- ✅ `discontinuedAt`, `discontinuedBy`, `discontinueReason`
- ✅ `notes`
...
**Status:** ❌ **NOT IMPLEMENTED**  
**Priority:** 🔴 **CRITICAL**  
**Database Schema:** ✅ Exists in Prisma (`Invoice` model)
...
**Database Fields (from schema):**
- ✅ `id`, `invoiceNumber`, `patientId`, `visitId`
- ✅ `items` (JSON array)
- ✅ `subtotal`, `discount`, `discountReason`, `tax`, `taxRate`, `total`
- ✅ `amountPaid`, `balance`, `status` (UNPAID, PARTIAL, PAID)
- ✅ `payments` (JSON array)
- ✅ `billedBy`, `billedAt`
- ✅ `notes`
...
- ❌ Report caching for performance (future enhancement)
...
- ❌ `exportToPdf()` - Export report to PDF (future)
- ❌ `exportToExcel()` - Export report to Excel (future)
...
**Status:** ❌ **NOT IMPLEMENTED**  
**Priority:** 🟡 **HIGH**  
**Database Schema:** ✅ Exists in Prisma (`AuditLog` model)
...
**Database Fields (from schema):**
- ✅ `id`, `userId`, `action` (AuditAction enum)
- ✅ `entityType`, `entityId`
- ✅ `details` (JSON), `changes` (JSON - before/after)
- ✅ `ipAddress`, `userAgent`
- ✅ `createdAt`
...
**Status:** ❌ **NOT IMPLEMENTED**  
**Priority:** 🟡 **HIGH**  
**Database Schema:** ⚠️ Needs metadata storage (can use JSON fields or separate model)
...
**Note:** For MVP, we can store file metadata in JSON fields in existing models (Patient.photoUrl, Visit.attachments). For production, consider a separate File model.
